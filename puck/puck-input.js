...
// Keeping track of breathing state
const states = {
    INHALE: "Inhale",
    EXHALE: "Exhale",
    HOLD: "Hold",
...
var linearAccel = {x: 0, y: 0, z: 0};

// Data processing to determine breathing
var prev_long_vals = [], prev_short_vals = [];
var window_seconds = 1;
var window_nvals = hz*window_seconds;
var short_nvals = 3;
var avg = 0;
var longAvg = 0;
var stdDev = 0;
...
// Thresholds
const maxLinAcc = 0.25;     // Max linear acceleration (abs value). If linAcc not between +/- this value, go to UNKNOWN state
const minLinAcc = 0.015;    // Min linear acceleration (abs value) to be considered start of a breath. Ignore values lower than this that are outside stddev.
const maxGyro = 50;         // Max/min degrees per second. If gyro not between these values, go to UNKNOWN state
...
function onLine(line) 
{
    ...
    if (d.length==7 && d[0]=="A") 
    {
        ...
        gyro.x = (parseInt(d[4])/100)/gyroScale;
        gyro.y = (parseInt(d[5])/100)/gyroScale;
        gyro.z = (parseInt(d[6])/100)/gyroScale;

        // Update the Madgwick filter
        madgwick.update(degrees_to_radians(gyro.x), degrees_to_radians(gyro.y), degrees_to_radians(gyro.z), accel.x, accel.y, accel.z, 0, 0, 0, deltaTime);
        ...
        // Get the linear acceleration in m/s^2
        linearAccel.x = (accel.x - gX);
        linearAccel.y = (accel.y - gY);
        linearAccel.z = (accel.z - gZ);

        // Get an average of the most recent 3 points
        prev_short_vals.push(linearAccel.z);
        if (prev_short_vals.length > short_nvals) {
            prev_short_vals.shift();
        }
        avg = average(prev_short_vals);

        // If we have some history, calculate the long average
        if (prev_long_vals.length > 0) {

            // Set the long
            longAvg = average(prev_long_vals);
            stdDev = standardDeviation(prev_long_vals);

            // If we have noise/movement, set state to UNKNOWN. Movement/noise is determined by thresholding the 3rd std dev
            if (longAvg + stdDev*3 > maxLinAcc || Math.abs(gyro.x) > maxGyro || Math.abs(gyro.y) > maxGyro || Math.abs(gyro.z) > maxGyro) {
            ...
            // If not, determine state based on data and current state
                ...
                // If our state is unknown but we got to here, set it to HOLD since the data should be steady
                if (state == states.UNKNOWN) {
                ...
                // In this state, we're watching for an inhale or exhale
                else if (state == states.HOLD) {
                    // Make sure we make the min threshold to be considered the start of a breath
                    if (Math.abs(avg-longAvg) > minLinAcc) {

                        // Check if the value is outside stddev*2.5 (positive is inhale, negative is exhale)
                        if (avg > (longAvg + stdDev*2.5) ) {
                            state = states.INHALE;
                ...
                // State is inhale for 1 second, then go to HOLD
                ...
                // State is exhale for 1 second, then go to HOLD
                else if (state == states.EXHALE) {
                    if (state_count < hz) {
                        state_count += 1;
                    }
                    else {
        ...
        // Add the linear accel z to the window
        prev_long_vals.push(linearAccel.z);
        if (prev_long_vals.length > window_nvals) {
            prev_long_vals.shift();
        }
    }
}
...
function release_button_up() {
    set_button(UP_BUTTON, false);
}

/* Rapid fire when tilted down */
function button_up() {
    release_button_down();
    set_button(UP_BUTTON, true);
}

function release_button_down() {
    set_button(DOWN_BUTTON, false);
}
